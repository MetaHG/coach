...
OpeEstimationCustomWIS = namedtuple("OpeEstimationCustom", ['ips', 'dm', 'dr', 'seq_dr', 'wis', 'wi_sum_across_episodes', 'wi_per_episode', 'wi_per_transition'])
OpeEstimationCustomSeqDR = namedtuple("OpeEstimationCustom", ['ips', 'dm', 'dr', 'seq_dr', 'wis', 
'seq_dr_per_episode', 'seq_dr_per_transition', 'rho_per_transition', 'reward_per_transition', 'v_value_per_transition', 'q_value_per_transition'])


class CustomOpeManager(OpeManager):
    def __init__(self):
        super().__init__()
    ...
    def _prepare_ope_shared_stats(self, evaluation_dataset_as_transitions: List[Transition], batch_size: int,
        ...
        for i in range(math.ceil(len(evaluation_dataset_as_transitions) / batch_size)):
            ...
            # last_probs = all_policy_probs[-1] 
            last_probs = self.all_behavior_cloning_model_probs[i] 

            all_v_values_reward_model_based.append(np.sum(last_probs * self.all_reward_model_rewards[i],
                                                          axis=1))
            all_v_values_q_model_based.append(np.sum(last_probs * q_values, axis=1))

            for j, t in enumerate(batch):
                t.update_info({
                    'q_value': q_values[j],
                    'softmax_policy_prob': last_probs[j], #all_policy_probs[-1][j],
                    'v_value_q_model_based': all_v_values_q_model_based[-1][j],

                })

        all_policy_probs = np.concatenate(self.all_behavior_cloning_model_probs, axis=0) # np.concatenate(all_policy_probs, axis=0)
    ...
    def gather_static_shared_stats(self, evaluation_dataset_as_transitions: List[Transition], batch_size: int,
                                   reward_model: Architecture, behavior_cloning_model: Architecture, network_keys: List) -> None:
        all_behavior_cloning_model_probs = []
        ...
        for i in range(math.ceil(len(evaluation_dataset_as_transitions) / batch_size)):
            ...
            all_behavior_cloning_model_probs.append(behavior_cloning_model.predict(batch_for_inference.states(network_keys)))
        ...
        self.all_behavior_cloning_model_probs = all_behavior_cloning_model_probs
    ...
    def evaluate_custom(self, evaluation_dataset_as_episodes: List[Episode], evaluation_dataset_as_transitions: List[Transition], batch_size: int,
                 discount_factor: float, q_network: Architecture, network_keys: List) -> OpeEstimation:
        """
        Run all the OPEs and get estimations of the current policy performance based on the evaluation dataset.
        ...
        ips, dm, dr = self.doubly_robust.evaluate(ope_shared_stats)
        #seq_dr = self.sequential_doubly_robust.evaluate(evaluation_dataset_as_episodes, discount_factor)
        seq_dr, seq_dr_per_episode, seq_dr_per_transition, rho_per_transition, reward_per_transition, v_value_per_transition, q_value_per_transition \
            = self.sequential_doubly_robust.evaluate_custom(evaluation_dataset_as_episodes, discount_factor)
        
        wis = self.weighted_importance_sampling.evaluate(evaluation_dataset_as_episodes)
        #wis, wi_sum_across_episodes, wi_per_episode, wi_per_transition = self.weighted_importance_sampling.evaluate_custom(evaluation_dataset_as_episodes)
        #return OpeEstimationCustomWIS(ips, dm, dr, seq_dr, wis, wi_sum_across_episodes, wi_per_episode, wi_per_transition)

        return OpeEstimationCustomSeqDR(ips, dm, dr, seq_dr, wis, seq_dr_per_episode, seq_dr_per_transition, rho_per_transition, reward_per_transition, v_value_per_transition, q_value_per_transition)
