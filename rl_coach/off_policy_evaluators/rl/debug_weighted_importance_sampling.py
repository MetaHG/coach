...
class DebugWeightedImportanceSampling(object):
# TODO add PDIS    
    @staticmethod
    def evaluate(evaluation_dataset_as_episodes: List[Episode]) -> (float, float, List[float], List[float]):
        """
        ...
        for episode in evaluation_dataset_as_episodes:
            ...
            for transition in episode.transitions:
                transition_weight = transition.info['softmax_policy_prob'][transition.action] / \
                      transition.info['all_action_probabilities'][transition.action]
                
                w_i *= transition_weight
                all_transition_weights.append(transition_weight)
        ...
        return wis, total_w_i_sum_across_episodes, per_episode_w_i, all_transition_weights
