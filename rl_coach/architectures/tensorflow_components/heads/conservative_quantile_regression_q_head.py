...
class ConservativeQuantileRegressionQHead(QHead):
    ...
    def _build_module(self, input_layer):
        ...
        ### Add the CQL Loss
        # self.actions contains actually tuples of (action index, action). We only want the actions below.
        replay_action_one_hot = tf.one_hot(self.actions[:, 1], self.num_actions, tf.cast(1.0, dtype=tf.float64), tf.cast(0.0, dtype=tf.float64), dtype=tf.float64,  name='action_one_hot')
